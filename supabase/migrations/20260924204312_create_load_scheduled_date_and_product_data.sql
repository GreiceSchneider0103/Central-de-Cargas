-- create_load_with_items: grava a data agendada informada ao criar a carga
-- (antes ela era descartada e a carga não aparecia na Agenda) e completa
-- fornecedor, peso e medidas dos itens com o cadastro do produto quando não
-- vierem no item (a criação da carga agora pede só SKU, nome e quantidade).
...
begin
  ...
  insert into public.loads (
    ...
    v_tipo_coleta_id,
    v_transportador_id,
    nullif(p_load->>'data_agendada', '')::timestamptz,
  ...
  loop
    ...
    insert into public.load_items (
      ...
      v_nome,
      v_quantidade,
      coalesce(v_fornecedor_id, v_product.fornecedor_id),
      v_cmv,
      coalesce(nullif(v_item->>'peso', '')::numeric, v_product.peso),
      coalesce(nullif(v_item->>'altura', '')::numeric, v_product.altura),
      coalesce(nullif(v_item->>'largura', '')::numeric, v_product.largura),
      coalesce(nullif(v_item->>'profundidade', '')::numeric, v_product.profundidade),
