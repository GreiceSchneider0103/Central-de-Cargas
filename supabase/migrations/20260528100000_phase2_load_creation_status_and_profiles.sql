-- Phase 2: safe load creation, load status standardization and admin profile helpers.

create or replace function public.is_valid_load_status(p_status text)
returns boolean
language sql
immutable
as $$
...
alter table public.loads
  drop constraint if exists chk_loads_status_allowed;

alter table public.loads
  add constraint chk_loads_status_allowed
  check (public.is_valid_load_status(status)) not valid;
...
-- Force direct load creation through create_load_with_items so headers cannot be created without items.
