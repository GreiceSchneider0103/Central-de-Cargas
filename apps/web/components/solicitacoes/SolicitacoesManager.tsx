...
import type { UserProfile, UserProfileRole } from '@/lib/auth/roles';
...
type RequestRow = { id: string; codigo: string; tipo: string; status: string; observacoes?: string | null; created_at: string };
...
export function SolicitacoesManager({ profile }: { profile: UserProfile }) {
  ...
  async function load() {
    let query = supabase.from('load_requests').select('id,codigo,tipo,status,observacoes,created_at,solicitante_id,loja_destino_id').order('created_at', { ascending: false });
    const role = profile.perfil;
    if (role === 'gerente_ecommerce') query = query.or('tipo.eq.FULL_MARKETPLACE,solicitante_id.eq.' + profile.id);
    if (role === 'vendedor_loja') query = query.or('solicitante_id.eq.' + profile.id + ',loja_destino_id.eq.' + (profile.loja_id ?? ''));
  ...
  async function addItemRow() {
    setItems((prev) => [...prev, { sku: '', nome_produto: '', quantidade: 1, cmv_unitario: 0, cmv_total: 0 }]);
  }
  ...
  async function createRequest() {
    ...
    if (profile.perfil === 'gerente_ecommerce' && tipo !== 'FULL_MARKETPLACE') return setError('Gerente e-commerce cria apenas solicitações Full.');
    if (profile.perfil === 'vendedor_loja' && tipo !== 'LOJA_FISICA') return setError('Vendedor cria apenas solicitações para loja física.');
    ...
    for (const item of items) {
      if (!item.sku || !item.nome_produto || !item.quantidade) return setError('Cada item exige SKU, nome e quantidade.');
    }

    const { data: me } = await supabase.from('users_profile').select('id').eq('auth_user_id', (await supabase.auth.getUser()).data.user?.id!).single();
    const code = `REQ-${new Date().getFullYear()}-${String(Date.now()).slice(-6)}`;

    const { data: req, error } = await supabase.from('load_requests').insert({
      codigo: code,
      tipo,
      empresa_id: empresaId || null,
      canal_id: canalId || null,
      marketplace_id: marketplaceId || null,
      destino_full_id: destinoFullId || null,
      loja_destino_id: lojaDestinoId || null,
      prioridade,
      status: 'Pendente',
    ...
    const insertItems = items.map((i) => ({ ...i, request_id: req.id }));
    const { error: itemErr } = await supabase.from('load_request_items').insert(insertItems);
    ...
    <div className="space-y-6">
      <div className="bg-white border rounded-xl p-4 space-y-3">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-2">
          ...
          <input placeholder="Empresa ID" className="h-10 border rounded px-2" value={empresaId} onChange={(e) => setEmpresaId(e.target.value)} />
          <input placeholder="Canal ID" className="h-10 border rounded px-2" value={canalId} onChange={(e) => setCanalId(e.target.value)} />
          <input placeholder="Marketplace ID" className="h-10 border rounded px-2" value={marketplaceId} onChange={(e) => setMarketplaceId(e.target.value)} />
          <input placeholder="Destino Full ID" className="h-10 border rounded px-2" value={destinoFullId} onChange={(e) => setDestinoFullId(e.target.value)} />
          <input placeholder="Loja destino ID" className="h-10 border rounded px-2" value={lojaDestinoId} onChange={(e) => setLojaDestinoId(e.target.value)} />
        </div>
        {items.map((item, idx) => (
          <div key={idx} className="grid grid-cols-1 md:grid-cols-6 gap-2">
            ...
            <input placeholder="Fornecedor ID" className="h-10 border rounded px-2" value={item.fornecedor_origem_id || ''} onChange={(e) => updateItem(idx, 'fornecedor_origem_id', e.target.value)} />
        ...
        <div className="flex gap-2">
          <button className="px-3 py-2 border rounded" onClick={addItemRow}>+ Item</button>
          ...
          <tbody>
            {rows.map((r) => (
              <tr key={r.id} className="border-b">
                <td>{r.codigo}</td><td>{r.tipo}</td><td>{r.status}</td><td>{new Date(r.created_at).toLocaleString('pt-BR')}</td>
