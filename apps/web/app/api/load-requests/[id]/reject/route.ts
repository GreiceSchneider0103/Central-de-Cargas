...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    await supabase.from('load_requests').update({ status: 'Recusada', motivo_recusa: motivo }).eq('id', id);
    await supabase.from('load_request_history').insert({ request_id: id, acao: 'REQUEST_REJECTED', status_anterior: current.status, status_novo: 'Recusada', observacao: motivo, autor_profile_id: profile.id });
